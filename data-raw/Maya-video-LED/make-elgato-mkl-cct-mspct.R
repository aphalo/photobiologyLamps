...
new.names <- "elgato_mkl_cct.mspct"
lamp.info <- "Elgato Mini Key Light"
...
for (i in seq_along(temp.mspct)) {
  ...
  member.names <- c(member.names, what.measured)
...
names(elgato_mkl_cct.mspct) <- member.names
summary(elgato_mkl_cct.mspct)
what_measured(elgato_mkl_cct.mspct)

save(elgato_mkl_cct.mspct, file = "data/elgato-mkl-cct-mspct.rda")

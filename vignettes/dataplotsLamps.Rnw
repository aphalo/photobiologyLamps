...
spectrum.spct <- get(paste(lamp.name, ".spct", sep=""))
spectrum.spct <- trim_spct(spectrum.spct, waveband(c(w.low, w.high)), fill = NA)
e2q(spectrum.spct, byref=TRUE)
plot(spectrum.spct, unit="energy")
plot(spectrum.spct, unit="photon")
}
@

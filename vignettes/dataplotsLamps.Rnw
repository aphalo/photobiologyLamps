...
%\VignetteDepends{knitr, photobiology, photobiologyLamps}
...
\title{\PBLam Version 0.1.4\\ Catalogue of Lamps}
...
e.spectrum.data <- calc_lamp_output(w.length.out=w.length.out, lamp.name=lamp.name, unit.out="energy", scaled=scaled)
...
<<plot-UVB-emperature, fig.width=8, fig.height=4, out.width='.95\\textwidth'>>=
fig_temp <- ggplot(data=qpanel.uvb313.temperature.data, aes(x=w.length, y=s.e.irrad, colour=factor(temperature)))
fig_temp <- fig_temp + geom_line() + labs(x="Wavelength (nm)", y="Spectral energy irradiance (W m-2)", colour="Temperature (C)")
print(fig_temp)
@

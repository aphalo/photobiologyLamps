...
spectrum.data <- get(paste(lamp.name, ".spct", sep=""))
spectrum.data <- e2q(trim_spct(spectrum.data, high.limit = w.high, fill = NA))
fig_energy <- ggplot(aes(x=w.length, y=s.e.irrad), data=spectrum.data) + 
  labs(x="Wavelength (nm)", y="Spectral energy irradiance\n(rel. units)", 
       title=lamp.name) + 
  geom_line() + stat_peaks(ignore_threshold=0.33, colour="red")
fig_photon <- ggplot(aes(x=w.length, y=s.q.irrad), data=spectrum.data) + 
...
fig_temp <- ggplot(data=qpanel.uvb313.temperature.dt, 
...
<<plot-white>>=
white.lamps <- c("incandescent.60w", "osram.36w.25", "philips.pls11w.827", 
                 "philips.tld36w.83", "philips.tld36w.865", "philips.tld36w.89", 
...
<<FEL>>=
FEL.spct <- FEL_spectrum(250:900)
D2.spct <- D2_spectrum(250:900)
